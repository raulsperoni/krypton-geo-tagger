...
class GeoEngine(object):
    ...
    def findSolutions(self,elements,solutions):
        ...
        self.findSameTypeSolutions(elements,solutions,solution_index)
        ...
        for geo_type_a in elements.keys():
            for geo_type_b in elements.keys():
                if geo_type_a != geo_type_b:
                    #logger.debug('%s || %s',geo_type_a,geo_type_b)
                    for element_id_a in elements[geo_type_a].keys():
                        element_a = elements[geo_type_a][element_id_a]
                        tokens_a = element_a['tokens']
                        for element_id_b in elements[geo_type_b].keys():
                            element_b = elements[geo_type_b][element_id_b]
                            tokens_b = element_b['tokens']
                            if (any([token._.part_of_solution for token in tokens_a]) and any([token._.part_of_solution for token in tokens_b])):
                                #logger.debug('PART OF SOLUTION EXIT %s || %s',tokens_a,tokens_b)
                                break
            ...
            for element_id_a in elements[geo_type].keys():
                ...
                for element_id_b in elements[geo_type].keys():
                    if element_id_a != element_id_b:
                        ...
                        if not self.inSolutionIndex(solution_index,search_term_a,search_term_b) and element_a['codigo'] != element_b['codigo'] and not self.shareToken(element_a,element_b):
                            shape_a = asShape(element_a['geometry'])
                            shape_b = asShape(element_b['geometry'])
                            ...
                            if shape_a.intersects(shape_b):
                                ...
                                for shape in shapes_found:
                                    ...
                                    if shape_found_before:
                                        logger.debug('DUPLICATE FOUND: %s, %s',element_a['tokens'],element_b['tokens'])
                                        break
                                ...
                                if not shape_found_before:
                                    ...
                                    solutions.append(solution)
                                    self.storeInSolutionIndex(solution_index,search_term_a,search_term_b)
                                    #self.printSolution(solution)

    def buildSolution(self,elements,intersection):
        #Mark tokens in solution as used
        for element in elements:
            for token in element['tokens']:
                token._.part_of_solution = True
        ...
        return {'centroid':list(intersection.coords),'elements': elements,'score':score, 'show':'/'.join([element['nombre'] for element in elements])}
    ...
    def inSolutionIndex(self,solution_index, a,b):
        return (solution_index.get(a,False) and solution_index[a].get(b)) or (solution_index.get(b,False) and solution_index[b].get(a))
